...
internal class ControllerSettings : ModSettingsCategory
{
	...
	public ControllerSettings(string filePath, LanguageConfig lng) : base("Controller", filePath, lng)
	{
		_rightStickDeadzone = CreateEntry(
			"RStickDeadzone", 0.15f,
			"Accepts Right Stick input only when its value is higher than this percentage of the range"
		);

		_leftStickDeadzone = CreateEntry(
			"LStickDeadzone", 0.15f,
			"Accepts Left Stick input only when its value is higher than this percentage of the range"
		);

		_leftTriggerDeadzone = CreateEntry(
			"LTriggerDeadzone", 0.1f,
			"Accepts Left Trigger input only when its value is higher than this percentage of the range"
		);

		_rightTriggerDeadzone = CreateEntry(
			"RTriggerDeadzone", 0.1f,
		...
		_toggleCamStateButton = CreateEntry(
			"ToggleCamState", ControllerButton.LB,
		...
		_snapAlignCamButton = CreateEntry(
			"SnapAlignCam", ControllerButton.RStick,
