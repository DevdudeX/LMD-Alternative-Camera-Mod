...
internal class KeyboardSettings : ModSettingsCategory
{
	...
	public KeyboardSettings(string filePath, LanguageConfig lng) : base("Keyboard", filePath, lng)
	{
		_originalCamKey = CreateEntry(
			"OriginalCam", KeyCode.Alpha1,
			"Key to set camera to original camera work"
		);

		_thirdPersonCamKey = CreateEntry(
			"ThirdPersonCam", KeyCode.Alpha2,
			"Key to set camera to third person view"
		);

		_firstPersonCamKey = CreateEntry(
			"FirstPersonCam", KeyCode.Alpha3,
			"Key to set camera to first person view"
		);

		_toggleCamStateKey = CreateEntry(
			"ToggleCamState", KeyCode.Tab,
		...
		_toggleCamAlignModeKey = CreateEntry(
			"ToggleCamAlignMode", KeyCode.Alpha5,
			"Key to switch between Auto and Manual alignment, see Camera/CamAlignMode)"
		);

		_snapAlignCamKey = CreateEntry(
			"SnapAlignCam", KeyCode.Space,
			"Key to align the camera behind the bike on demand\n" +
			"None to disable, otherwise any valid keyboard key (see also Controller)"
		);

		_invertCamAlignModeKey = CreateEntry(
			"InvertCamAlignMode", KeyCode.RightControl,
