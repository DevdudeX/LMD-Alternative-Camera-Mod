...
internal class CameraSettings : ModSettingsCategory
{
	...
	public CameraSettings(string filePath, LanguageConfig lng) : base("Camera", filePath, lng)
	{
		_initialMode = CreateEntry(
			"InitialMode", CameraView.ThirdPerson,
			"Camera mode to begin with: Original, ThirdPerson, FirstPerson"
		);

		_alignmentMode = CreateEntry(
			"AlignMode", CameraAlignmentMode.Auto,
			"* Auto  : camera follows the bike automatically\n" +
			"* Manual: camera can be aligned respective to the manual align mode"
		);

		_manualAlignmentInput = CreateEntry(
			"ManualAlign", CameraManualAlignmentInput.MouseOrRStick,
		...
		_autoAlignSpeed = CreateEntry(
			"AutoAlignSpeed", 1.80f,
		...
		_thirdPersionRotationVert = CreateEntry(
			"ThirdPersonCamPosVertical", -20f,
			"Default vertical position of the camera above behind the bike"
		);

		_thirdPersionInitialFollowDistance = CreateEntry(
			"ThirdPersonInitialFollowDistance", 4.5f,
