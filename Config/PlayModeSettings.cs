...
internal class PlayModeSettings : ModSettingsCategory
{
	...
	public PlayModeSettings(string filePath, LanguageConfig lng) : base("PlayMode", filePath, lng)
	{
		_gameHudVisible = CreateEntry(
			"GameHudVisible", true,
			"Initial visibility of the game hud with timer and track info"
		);
		
		_modHudInfoDisplay = CreateEntry(
			"ModHudInfoDisplay",
		...
		_modHudTextSize = CreateEntry(
			"ModHudTextSize", 20,
			"Size of the mod hud text"
		);
		
		_showCamInstructionsInPauseMenu = CreateEntry(
			"ShowCamInstructionsInPauseMenu", true,
			"Visibility of the camera instructions in pause menu"
		);

		_enableToggleCamStateByOriginalCamKey = CreateEntry(
			"EnableToggleCamStateByOriginalCamKey", true,
