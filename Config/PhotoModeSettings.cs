...
internal class PhotoModeSettings : ModSettingsCategory
{
   ...
   public PhotoModeSettings(string filePath, LanguageConfig lng) 
      : base("PhotoMode", filePath, lng)
   {
      _cameraMovementSpeed = CreateEntry("CameraMovementSpeed", 1.8f,
         "Basic speed of the camera movement");
      _cameraMovementSpeedMultiplier = CreateEntry("CameraMovementSpeedMultiplier", 8f,
         "Speed multiplier applied when the according key/button is pressed to accelerate");
      _autoHideHud = CreateEntry("AutoHideHud", false,
         "Tells whether game hud should be auto-hidden when entering photo mode");
      _screenshotFolder = CreateEntry("ScreenshotFolder",
      ...
      _screenshotJpgQuality= CreateEntry("ScreenshotJpgQuality", 75,
