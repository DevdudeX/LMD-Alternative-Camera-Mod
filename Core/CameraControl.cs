...
internal class CameraControl
{
	...
	private Vector3 _targetOffset = new(0f, 2.4f, 0f);
	...
	public void ProcessBikeMode()
	{
		...
		// Clamp the up-down rotation to reasonable range
		_rotationVertical = ClampAngle(
			_rotationVertical,
			_verticalClampAnglesMap[_currentCamView].Item1,
			_verticalClampAnglesMap[_currentCamView].Item2
		);
	...
	public IEnumerator TakeScreenshot()
	{
		...
		try
		{
			...
			Texture2D imageOverview = new(
				gameCam.targetTexture.width,
	...
	private void ApplyThirdPersonCam()
	{
		...
		ApplyCameraSettings(
			_cfg.Camera.ThirdPersionInitialFollowDistance.Value,
	...
	private void ApplyCamPos(CamPos cPos)
	{
		_camTransform.eulerAngles      = cPos.EulerAngles;
		_camTransform.localEulerAngles = cPos.LocalEulerAngles;
		_camTransform.position         = cPos.Position;
		_rotationHorizontal            = cPos.RotationHorizontal;
		_rotationVertical              = cPos.RotationVertical;
		_rotationRoll                  = cPos.RotationRoll;
	}


	private void SaveCamPos(CamPos cPos)
	{
		cPos.EulerAngles        = _camTransform.eulerAngles;
		cPos.LocalEulerAngles   = _camTransform.localEulerAngles;
		cPos.Position           = _camTransform.position;
		cPos.RotationHorizontal = _rotationHorizontal;
		cPos.RotationVertical   = _rotationVertical;
		cPos.RotationRoll       = _rotationRoll;
	}
