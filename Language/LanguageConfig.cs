...
internal class LanguageConfig
{
	...
	{
		string langDir = GetLanguageDir();
		string[] langIniFiles = Directory.GetFiles(langDir, "lang.*.ini");
		List<string> langs = new();
		foreach (string iniFile in langIniFiles)
		{
			string[] parts = iniFile.Split('.');
			if (parts.Length == 3)
			{
	...
	public static LanguageConfig Load(string? langCode = null)
	{
		...
		string filePath = Path.Combine(langDir, "lang." + lc + ".ini");
